...
class PaymentService {
  ...
  async initiateWithdrawal(phoneNumber, amount, userId, challengeId, isRefund = false) {
    try {
      ...
      // Generate unique request ID
      const requestId = `${isRefund ? 'REF' : 'PAY'}_${challengeId}_${userId}_${Date.now()}`;
  ...
  async processMatchResult(matchResult, challenge) {
    try {
      ...
      const { challenger, opponent, bet_amount } = challenge;
      ...
      if (drawResults.includes(resultType)) {
        ...
        // Refund both players
        await this.initiateWithdrawal(challenge.challenger_phone, bet_amount, challenger, challenge.id, true);
        await this.initiateWithdrawal(challenge.opponent_phone, bet_amount, opponent, challenge.id, true);
      ...
      await this.initiateWithdrawal(winnerPhone, winAmount, winnerUserId, challenge.id, false);
